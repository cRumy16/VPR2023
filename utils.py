...
class ProxyHead(nn.Module): # Funge da fully connect (dimensionlity reduction) 
...
class Proxy: #Qui si definisce una struttura dati per i descriptors. I descriptors con stessa label vengono 
    # mediati (si fa la media). Che passa appunto nel fully connect sopra. 
    def __init__(self, tensor = None, n = 1, dim = 512):
        if tensor is None:
            self.__arch = torch.zeros(dim) # __ -> convenzione di python (dounder) 
        else:
            self.__arch = tensor
        self.__n = n
    ...
    def __add__(self, other):
        return ProxyBank.Proxy(tensor=self.__arch + other.__arch, n=self.__n + other.__n) #aggioranemnto della lista

class ProxyBank: #Dizionario che per ogni chiave (id luogo) corrisponde il proxy di tale luogo. 
    ...
    def update_bank(self, proxies, labels): #aggiornamento del dizionario  
        #riempo la banca
        for d, l in zip(proxies, labels):
            ...
            if l.item not in self.__bank:
                ...
                self.__bank[l.item()] = self.__bank[l.item()] + Proxy(d) # In questo caso è una specie di appena fra numeri e lista

    def update_index(self): # fa la media fra ciò che è stato salvato prima (descriptor per label) e il numero di elementi sommati( dividendo viene la media)
        self.index.reset() # Serve pulirlo ogni volta. 
    ...
    # la banca viene trasformata in un index per usarla nel faiss e va ripulita sempre. (altrimenti si ricostruirebbe lo stesso batch ogni volta) 
    def reset(self):
         del self.__bank
         del self.index
         self.__bank = {}
         self.__base_index = faiss.IndexFlatL2( self.proxy_dim )
         self.index = faiss.IndexIDMap( self.__base_index ) # Struttura dati per il knn dopo
    ...
    def batch_sampling(self , batch_dim):
        ...
        while len(self.__bank) >= batch_dim:
            ...
            # Cerca tra i vari batch (dentro la stessa epoch) quelli più vicini, e li mette nel batch sampler. che verrà sfruttata l'epoch dopo
            _, batch = self.index.search(start_proxy.reshape(1,-1).detach().cpu(), batch_dim)
            # From [[]] to []: only one row needed
            batch = batch[0] # Conversione da matrice a riga (la matrice è di fatto una sola riga) 
    ...
    #Una volta ordinato tutte i batch generati, prende i batch più simili e li butta nella epoch nuova. 
class ProxyBankBatchSampler(Sampler):
    def __init__(self, dataset, batch_size, bank):
        ...
        self.counter = 0
    def __iter__(self): 
        if self.is_first_epoch and self.counter % 2 == 0: # perché c'è numworkers = 2 pertanto è come se venisse eseguito due volte, quindi lo facciamo solo una volta. 
            self.is_first_epoch = False
            random_indeces_perm = torch.randperm(len(self.dataset))
            indeces =  torch.split(random_indeces_perm , self.batch_size) #suddivido per batchsize 
...
def compute_recalls(eval_ds: Dataset, queries_descriptors : np.ndarray, database_descriptors : np.ndarray,
    ...
    if num_preds_to_save != 0:
        ...
        visualizations.save_preds(predictions[:, :num_preds_to_save], eval_ds, "/kaggle/working", save_only_wrong_preds)
